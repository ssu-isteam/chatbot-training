...
fun main2(args:Array<String>){
 ...
 //  packedRawDataSet.rawDataSets = ArrayList( packedRawDataSet.rawDataSets.subList(0,100))
    ...
    var dataSource = AutoencoderDataSource(packedRawDataSet = packedRawDataSet, ktfid = koreanTfidfVectorizer, batchSize = batchSize)

    var progressBar = ProgressBar("Fitting autoencoder...", packedRawDataSet.rawDataSets.size.toLong()*epoch)
    ...
    for(i in 0 until epoch){
        while(dataSource.hasNext()){
            ...
            progressBar.stepBy(dataSource.currentCount)
        }
        dataSource.reset()
    }
